...
public class ProviderLoginRedirectController {
	...
	@ApiResponse(responseCode = "302", description = "Kakao Login Redirect", headers = @Header(name = "location", description = "Kakao Login Redirect Url", schema = @Schema(type = "string")))
	@GetMapping("/kakao")
	public ResponseEntity<Void> kakaoLoginRedirectUri() {
		return ResponseEntity.status(HttpStatus.FOUND).location(URI.create(kakaoLoginRedirectUri)).build();
	}
