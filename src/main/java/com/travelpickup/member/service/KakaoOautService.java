...
public class KakaoOautService {
	...
	public KakaoOautService(KakaoOauthWebClient kakaoOauthWebClient,
		...
		TravelPickupManagerRepository travelPickupManagerRepository, JWTUtil jwtUtil) {
	...
	public LoginResponseDto login(String codeValue) {
		...
		KakaoUserMeResponseDto kakaoUserMeResponseDto = kakaoOauthWebClient.getKakaoUserMe(
			kakaoTokenResponseDto.getAccess_token());

		Optional<TravelPickupUser> optionalTravelPickupUser = travelPickupUserRepository.findByProviderId(
			kakaoUserMeResponseDto.getId());

		if (optionalTravelPickupUser.isPresent()) {
			TravelPickupUser saveTravelPickupUser = optionalTravelPickupUser.get();
			return LoginResponseDto.of(
				jwtUtil.createJwt(saveTravelPickupUser.getId(), saveTravelPickupUser.getTravelPickupUserRole().name(),
					60 * 60 * 60 * 3600L));
		}
	...
	public LoginResponseDto adminLogin(String codeValue) {
		...
		Optional<TravelPickupManager> optionalTravelPickupManager = travelPickupManagerRepository.findByProviderId(
			kakaoUserMeResponseDto.getId());

		if (optionalTravelPickupManager.isPresent()) {
			TravelPickupManager saveTravelPickupManager = optionalTravelPickupManager.get();
			return LoginResponseDto.of(
				jwtUtil.createJwt(saveTravelPickupManager.getId(), saveTravelPickupManager.getCenterId(),
					saveTravelPickupManager.getTravelPickupManagerRole().name(), 60 * 60 * 60 * 3600L));
		}
