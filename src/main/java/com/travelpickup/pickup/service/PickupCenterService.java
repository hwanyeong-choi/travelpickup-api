...
public class PickupCenterService {
    ...
    public List<PickupCenterResponseDto> getPickupCenterList() {
        return pickupCenterRepository
                .findAll()
                .stream()
                .map(PickupCenterResponseDto::of)
                .toList();
    }

    @Transactional(readOnly = true)
    public PickupCenterResponseDto getPickupCenterById(Long pickupCenterId) {
        Optional<PickupCenter> optionalPickupCenter = pickupCenterRepository
                .findById(pickupCenterId);
