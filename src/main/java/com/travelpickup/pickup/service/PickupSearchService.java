...
import com.travelpickup.pickup.dto.response.DestinationLocationResponseDto;
...
import com.travelpickup.pickup.dto.response.PickupProductResponseDto;
...
public class PickupSearchService {
	...
	public PickupSearchService(PickupRepository pickupRepository, PickupCenterRepository pickupCenterRepository,
		DestinationLocationRepository destinationLocationRepository, PickupProductRepository pickupProductRepository,
		PickupProductImgRepository pickupProductImgRepository, AmazonS3Service amazonS3Service) {
	...
	public MyPickupResponseDto getMyPickup(Long userId) {
		...
		Map<String, List<PickupResponseDto>> pickupResponseMap = pickupResponseDtoList.stream()
			.collect(Collectors.groupingBy(pickupResponseDto -> {
				if (PickupState.getFinishStateList().contains(pickupResponseDto.getState()))
					return FINISH_PICKUP_KEY;
	...
	public PickupDetailResponseDto getPickup(Long userId, Long pickupId) {
		...
		DestinationLocation destinationLocation = destinationLocationRepository.findByPickupId(pickupId)
			.orElse(DestinationLocation.createEmpty());

		DestinationLocationResponseDto destinationLocationResponseDto = DestinationLocationResponseDto.of(
			destinationLocation);

		List<PickupProductResponseDto> pickupProductResponseDtoList = pickupProductRepository.findByPickupId(pickupId)
			.stream()
			.map(pickupProduct -> {
				Optional<PickupProductImg> optionalPickupProductImg = pickupProductImgRepository.findByPickupProductId(
					pickupProduct.getPickupProductId());

				if (optionalPickup.isPresent()) {
					PickupProductImg pickupProductImg = optionalPickupProductImg.get();

					try {
						String pickupProductImgByBase64 = amazonS3Service.getPickupImageFile(
							pickupProductImg.getPath());
		...
		return PickupDetailResponseDto.of(pickup, destinationLocationResponseDto, pickupProductResponseDtoList,
