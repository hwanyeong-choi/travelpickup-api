...
public class PickupCenterController {
	...
	@ApiResponse(responseCode = "200", description = "픽업센터 정보 리스트", content = @Content(mediaType = "application/json", schema = @Schema(type = "array", implementation = PickupCenterResponseDto.class)))
	@GetMapping
	public ResponseEntity<List<PickupCenterResponseDto>> getPickupCenterList() {
		...
		return ResponseEntity.status(HttpStatus.OK).body(pickupCenterResponseDtoList);
	}
	...
	public ResponseEntity<PickupCenterResponseDto> getPickupCenter(
		...
		return ResponseEntity.status(HttpStatus.OK).body(pickupCenterResponseDto);
	}
